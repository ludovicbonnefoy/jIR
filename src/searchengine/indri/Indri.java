...
public class Indri
{
	...
	//		Runtime runtime = Runtime.getRuntime();
	//		try{
	//			Process process = runtime.exec(new String[]{GetProperties.getInstance().getProperty("indriDirectory")+"/IndriRunQuery",GetProperties.getInstance().getProperty("tmpDirectory")+"/indriQuery_"+Thread.currentThread().getId()});
	//
	//			// Set a timer to interrupt the process if it does not return within the timeout period
	//			Timer timer = new Timer();
	//			timer.schedule(new InterruptScheduler(Thread.currentThread()), Integer.parseInt(GetProperties.getInstance().getProperty("timeout")));
	//
	//			try {
	...
	//				process.destroy();
	//				Log.getInstance().add(e);
	//				Log.getInstance().add(new TimeoutException("did not return results"));
	//			}
